...
namespace Bulky.BlobStorage.Adapter
{
	public sealed class BlobService(BlobServiceClient blobServiceClient) : IBlobStorage
	{
		...
		public async Task<string> UploadAsync(Stream stream, string container, string fileName)
		{
			...
			await blobContainerClient.GetBlobClient(fileName).UploadAsync(stream, overwrite: true);
